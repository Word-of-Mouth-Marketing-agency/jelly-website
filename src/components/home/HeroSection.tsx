...
export default function HeroSection({ locale }: { locale: string }) {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      // Stagger the three text children: h1, p, button row
      if (textEl) {
  ...
  return (
    <section className="w-full grid grid-cols-1 md:grid-cols-2 min-h-[400px] md:min-h-[600px]">
      ...
      <div
        ref={textRef}
        className="flex flex-col justify-center px-margin-mobile md:px-margin-desktop py-16 md:py-0 md:pl-[max(64px,calc(50vw-640px))]"
      >
